...
tags: ["ML", "Regression", "Ridge Regression", "Regularization", "Gradient Descent", "MAP"]
toc: true
...
3. If $0<\lambda<\infty$: $0\le\|\hat{\mathbf w}\|_2^2\le\|\hat{\mathbf w}_{LS}\|_2^2$.
...
$$
\mathbf y=H\mathbf w+\boldsymbol\varepsilon,\qquad
\mathbf y,\boldsymbol\varepsilon\in\mathbb R^N,\quad H\in\mathbb R^{N\times(D+1)},\quad\mathbf w\in\mathbb R^{D+1}.
$$

$$
\begin{aligned}
\operatorname{RSS}(\mathbf w)&=\sum_{i=1}^{N}(y_i-\mathbf h(\mathbf x_i)^T\mathbf w)^2
=(\mathbf y-H\mathbf w)^T(\mathbf y-H\mathbf w),\\
\|\mathbf w\|_2^2&=w_0^2+w_1^2+\cdots+w_D^2=\mathbf w^T\mathbf w,\\
J(\mathbf w)&=\operatorname{RSS}(\mathbf w)+\lambda\|\mathbf w\|_2^2
=(\mathbf y-H\mathbf w)^T(\mathbf y-H\mathbf w)+\lambda\mathbf w^T\mathbf w.
\end{aligned}
$$

이 식은 원본처럼 절편 $w_0$도 정규화한다. 절편을 제외하는 구현에서는 패널티 행렬을 달리 두어야 한다.


Cost function을 행렬로 나타내면 위 식과 같다.
...
\begin{aligned}
\nabla J(\mathbf w)
&=\nabla\big[(\mathbf y-H\mathbf w)^T(\mathbf y-H\mathbf w)+\lambda\mathbf w^T\mathbf w\big]\\
&=\nabla\big[(\mathbf y-H\mathbf w)^T(\mathbf y-H\mathbf w)\big]+\lambda\nabla(\mathbf w^T\mathbf w)\\
&=\underbrace{-2H^T(\mathbf y-H\mathbf w)}_{\text{fit term}}+\underbrace{2\lambda\mathbf w}_{\text{penalty term}}.
\end{aligned}
$$
...
$$
\begin{aligned}
-2H^T\mathbf y+2H^TH\mathbf w+2\lambda I\mathbf w&=0,\\
(H^TH+\lambda I)\mathbf w&=H^T\mathbf y,\\
\boxed{\hat{\mathbf w}=(H^TH+\lambda I)^{-1}H^T\mathbf y}.&
\end{aligned}
$$
...
$$
\nabla J(\mathbf w)=-2H^T(\mathbf y-H\mathbf w)+2\lambda\mathbf w.
$$

$\|\nabla J(\mathbf w^{(t)})\|\ge\epsilon$인 동안 $j=0,\ldots,D$에 대해

$$
\begin{aligned}
\mathrm{partial}_j&=-2\sum_{i=1}^{N}h_j(\mathbf x_i)\big[y_i-\hat y_i(\mathbf w^{(t)})\big],\\
w_j^{(t+1)}&\leftarrow w_j^{(t)}-\eta\big(\mathrm{partial}_j+2\lambda w_j^{(t)}\big)\\
&=(1-2\eta\lambda)w_j^{(t)}-\eta\,\mathrm{partial}_j.
\end{aligned}
$$

**표기 정정:** 원본의 종료 조건에 적힌 $\nabla\operatorname{RSS}$ 대신, 정규화항을 포함한 전체 목적함수의 $\nabla J$를 사용한다.
...
=\frac{\underbrace{p(X\mid\theta)}_{\text{likelihood}}\underbrace{p(\theta)}_{\text{prior}}}{p(X)}.
$$

$$
\hat\theta_{\mathrm{MLE}}=\underset\theta{\operatorname{argmax}}\,p(X\mid\theta),
\qquad
\hat\theta_{\mathrm{MAP}}=\underset\theta{\operatorname{argmax}}\,p(X\mid\theta)p(\theta).
$$

원본의 화살표처럼 MLE는 likelihood를, MAP는 likelihood와 prior의 곱을 최대화한다. $p(X)$는 $\theta$에 무관하다.
...
독립 Gaussian 오차와 독립 사전분포 $w_j\sim\mathcal N(0,\tau^2)$를 사용한다.

$$
\begin{aligned}
p(D\mid\mathbf w)&\propto\exp\!\left[-\frac1{2\sigma^2}\sum_i\left(y_i-\sum_jw_jh_j(\mathbf x_i)\right)^2\right],\\
p(\mathbf w)&\propto\exp\!\left[-\frac1{2\tau^2}\sum_jw_j^2\right],\\
\ln p(\mathbf w\mid D)&=C-\frac1{2\sigma^2}\operatorname{RSS}(\mathbf w)-\frac1{2\tau^2}\sum_jw_j^2.
\end{aligned}
$$

$C$는 $\mathbf w$와 무관한 상수다. 음의 로그에 양수 $2\sigma^2$를 곱해도 최소점은 같으므로

$$
\boxed{\hat{\mathbf w}_{\mathrm{MAP}}
=\underset{\mathbf w}{\operatorname{argmin}}\left[\operatorname{RSS}(\mathbf w)+\lambda\sum_jw_j^2\right]},
\qquad \lambda=\frac{\sigma^2}{\tau^2}>0.
$$

**표기 정정:** 로그 posterior는 상수 차이를 제외한 등식으로 썼다. 원본의 로그값·argmin 사이의 비례 기호는 위의 등식과 최소점 관계로 구분한다.
