...
**<center> $P(\vert \hat{\theta}_MLE - \theta^*\vert \geq \epsilon) \leq 2e^-2N\epsilon^2$</center>**<br>
...
**<center>${\hat{\mu}}_MLE = \frac{1}{n}\sum_{i=1}^{n}x_i , {\hat{\sigma}^2}_MLE = \frac{1}{n}\sum_{i=1}^{n}\left(x_i-\mu\right)^2$</center>**<br>
를 구할 수 있다.<br>
추가적으로 unbiased된 것은 다음과 같이 나타낸다.
**<center>${\hat{\sigma}^2}_unbiased = \frac{1}{n}\sum_{i=1}^{n-1}\left(x_i-\mu\right)^2$</center>**<br>