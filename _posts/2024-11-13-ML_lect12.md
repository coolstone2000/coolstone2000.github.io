...
tags: ["ML", "Classification", "Ensemble Learning", "Bagging", "Random Forest", "Boosting", "AdaBoost"]
toc: true
...
모든 표본의 초기 가중치를 $\alpha_i^{(1)}=1/N$으로 둔다. 반복 $t=1,\ldots,T$에서 가중치 $\alpha_i^{(t)}$로 약한 분류기 $f_t$를 학습하고, 가중 오류율과 분류기 계수를 구한다.

$$
\varepsilon_t=\sum_{i=1}^{N}\alpha_i^{(t)}\mathbf1\{f_t(\mathbf x_i)\ne y_i\},
\qquad \hat w_t=\frac12\ln\frac{1-\varepsilon_t}{\varepsilon_t}.
$$

$y_i,f_t(\mathbf x_i)\in\{-1,+1\}$일 때 정규화 전 가중치는

$$
\widetilde\alpha_i^{(t+1)}=
\begin{cases}
\alpha_i^{(t)}e^{-\hat w_t},& f_t(\mathbf x_i)=y_i,\\
\alpha_i^{(t)}e^{\hat w_t},& f_t(\mathbf x_i)\ne y_i
\end{cases}
=\alpha_i^{(t)}e^{-\hat w_t y_i f_t(\mathbf x_i)}.
$$

$$
\alpha_i^{(t+1)}=\frac{\widetilde\alpha_i^{(t+1)}}{\sum_r\widetilde\alpha_r^{(t+1)}},
\qquad
\boxed{\hat y=\operatorname{sign}\!\left(\sum_{t=1}^{T}\hat w_t f_t(\mathbf x)\right)}.
$$

원본의 “Normalize weight” 단계를 분모로 명시했다. 위 유한한 계수 식은 $0<\varepsilon_t<1$에서 정의되며, 일반적인 boosting 단계는 $\varepsilon_t<1/2$인 약한 분류기를 사용한다. 오류율 0 등의 경계는 별도 종료 처리가 필요하다.
