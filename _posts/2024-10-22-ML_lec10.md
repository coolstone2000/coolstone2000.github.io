...
tags: ["ML", "Classification", "Logistic Regression", "SGD", "Online Learning"]
toc: true
...
=\sum_{i=1}^{N}h_j(\mathbf x_i)
\left[\mathbf1\{y_i=+1\}-P(y=+1\mid\mathbf x_i,\mathbf w)\right].
$$

합은 $N$개 학습 표본을 순회하고, 한 표본의 선형 점수와 확률 계산은 $D$차원에서 $O(D)$다. 원본처럼 각 좌표마다 확률을 다시 계산하면 전체 비용이 $O(ND^2)$가 되지만, **확률을 표본당 한 번 계산해 모든 좌표에 재사용하면 전체 gradient를 $O(ND)$에 구할 수 있다.**
...
$\ell_i(\mathbf w)=\ln P(y_i\mid\mathbf x_i,\mathbf w)$라 하면 합으로 정의한 정규화 목적함수의 미분은

$$
\frac{\partial}{\partial w_j}\left[\sum_{i=1}^{N}\ell_i(\mathbf w)-\lambda\|\mathbf w\|_2^2\right]
=\sum_{i=1}^{N}\frac{\partial\ell_i}{\partial w_j}-2\lambda w_j.
$$

원본의 샘플별 갱신은 목적함수 전체를 $N$으로 나눈 평균 규약에 해당한다. 표본 $i$를 균일하게 뽑으면

$$
g_j^{(i)}=\frac{\partial\ell_i}{\partial w_j}-\frac{2\lambda}{N}w_j,
\qquad
\mathbb E_i[g_j^{(i)}]=\frac1N\sum_i\frac{\partial\ell_i}{\partial w_j}-\frac{2\lambda}{N}w_j.
$$

따라서 이 값은 **평균 목적함수**의 불편 gradient 추정량이다. 합 목적함수의 gradient를 추정하려면 샘플 항에도 $N$을 곱해 $N\,\partial\ell_i/\partial w_j-2\lambda w_j$로 쓴다.
