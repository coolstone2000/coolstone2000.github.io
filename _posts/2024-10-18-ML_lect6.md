...
tags: ["ML", "Regression", "LASSO", "Regularization", "Feature Selection", "Coordinate Descent"]
toc: true
...
$$
Z_j=\sqrt{\sum_{i=1}^{N}h_j(\mathbf x_i)^2},
\qquad
\widetilde h_j(\mathbf x_k)=\frac{h_j(\mathbf x_k)}{Z_j}.
$$

$Z_j$는 원본 화살표의 normalizer다. $Z_j>0$인 특징 열에 적용하면 $\sum_i\widetilde h_j(\mathbf x_i)^2=1$이 된다.
...
계수를 $\hat{\mathbf w}=0$ 또는 선택한 초기값으로 시작하고, 수렴할 때까지 $j=0,\ldots,D$를 순회한다. 여기서는 위에서 각 특징 열을 길이 1로 정규화했다고 가정한다.

$$
\begin{aligned}
\hat y_i^{(-j)}&=\sum_{k\ne j}\hat w_k\widetilde h_k(\mathbf x_i),\\
\rho_j&=\sum_{i=1}^{N}\widetilde h_j(\mathbf x_i)\underbrace{(y_i-\hat y_i^{(-j)})}_{\text{feature }j\text{ excluded residual}},\\
\hat w_j&\leftarrow\rho_j.
\end{aligned}
$$

이 식은 원본의 정규화된 최소제곱 coordinate update이며, LASSO의 soft-thresholding을 적용하기 전 단계다.
