...
tags: ["ML", "Classification", "Logistic Regression", "MLE", "Cross Entropy", "Softmax"]
toc: true
...
이진 분류에서 $p=P(y=+1\mid\mathbf x)$라 하면

$$
\operatorname{odds}(\mathbf x)=\frac{P(y=+1\mid\mathbf x)}{P(y=-1\mid\mathbf x)}
=\frac{p}{1-p}.
$$

원본의 판정 규칙은 odds가 1보다 크면 $\hat y=+1$, 그렇지 않으면 $\hat y=-1$이다. 이는 $p>1/2$와 같은 조건이며 경계의 동률은 별도 규약이다.
...
일단 많이 했봤듯이 $\triangledown l(w)=0$은 안되는 것을 알고 있을 것이다. 그래서 지금 최소값을 찾는게 아니라 최대값을 찾아야하니 gradient descent가 아닌 **gradient ascent**를 한다.

$$
L(\mathbf w)=\prod_{i=1}^{N}P(y_i\mid\mathbf x_i,\mathbf w),
\qquad \ell(\mathbf w)=\ln L(\mathbf w).
$$

$$
\boxed{\hat{\mathbf w}=\underset{\mathbf w}{\operatorname{argmax}}L(\mathbf w)
=\underset{\mathbf w}{\operatorname{argmax}}\ell(\mathbf w)}.
$$

원본의 $l$과 $ll$을 각각 likelihood $L$과 log-likelihood $\ell$로 구분해 썼다.
...
$p_i=P(y=+1\mid\mathbf x_i,\mathbf w)$라 두면

$$
\frac{\partial\ell(\mathbf w)}{\partial w_j}
=\sum_{i=1}^{N}\underbrace{h_j(\mathbf x_i)}_{\text{feature value}}
\underbrace{\big(\mathbf1\{y_i=+1\}-p_i\big)}_{\delta_i\,:\,\text{target minus prediction}}.
$$

| 실제 라벨 | $p_i\to1$일 때 $\delta_i$ | $p_i\to0$일 때 $\delta_i$ |
|---|---|---|
| $y_i=+1$ | $1-1=0$ | $1-0=1$ |
| $y_i=-1$ | $0-1=-1$ | $0-0=0$ |

원본 표의 계수 증가·감소 설명은 $h_j(\mathbf x_i)>0$인 경우에 해당한다. 실제 gradient 기여의 부호는 특징값과 $\delta_i$의 곱으로 결정된다.
...
=\frac1{1+e^{-\mathbf w^T\mathbf h(\mathbf x)}}
=\frac{e^{\mathbf w^T\mathbf h(\mathbf x)}}{1+e^{\mathbf w^T\mathbf h(\mathbf x)}}.
$$

클래스가 $1,\ldots,C$이면

$$
\boxed{P(y=c\mid\mathbf x,\mathbf w_1,\ldots,\mathbf w_C)
=\frac{e^{\mathbf w_c^T\mathbf h(\mathbf x)}}{\sum_{r=1}^{C}e^{\mathbf w_r^T\mathbf h(\mathbf x)}}}.
$$

분모의 합 인덱스는 선택한 클래스 $c$와 구별하도록 $r$로 썼다.
