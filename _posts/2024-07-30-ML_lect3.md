...
>**<center>Model: $y_{i} = w_{0}+w_{1}x_{i}+w_{2}sin(\frac{2\pi t_{i}}{12}) + +w_{3}cos(\frac{2\pi t_{i}}{12}) + \epsilon_{i}$</center><br>**
...
**<center>$\triangledown f(w) = (\frac{f(w)}{\partial w})^{T} + (\frac{f(w)}{\partial w^{T}})$</center><br>**
